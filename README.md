...
## v4.5 — modo web/desktop adaptado

- Em telas maiores, o Monitora deixa de parecer apenas um app mobile centralizado.
- A interface web ganha layout de desktop com:
  - coluna lateral para marca e clima;
  - navegação fixa lateral no lugar do rodapé móvel;
  - área principal mais ampla para conteúdo;
  - home reorganizada em duas colunas;
  - perfil em duas colunas;
  - mapa grande dentro do layout desktop.
- Em desktop, o mapa não entra mais no modo “fullscreen mobile”; ele ocupa uma tela ampla dentro da estrutura web.
- Em tablet horizontal, a Home já usa uma composição em duas colunas.


## v4.5.1 — refinamento UX para desktop

- Corrige o menu que ficava flutuando no meio da tela.
- Navegação desktop passa a ser vertical e integrada à lateral.
- Remove o botão “Registrar” flutuante da navegação e o transforma em ação lateral consistente.
- Home passa a seguir hierarquia de dashboard:
  1. saudação + ação principal;
  2. faixa compacta de situação/alertas/chuva;
  3. mapa principal + ocorrências na coluna lateral.
- Reduz espaços vazios e melhora alinhamento entre blocos.
- Alertas usam duas colunas no desktop.
- Perfil usa painel lateral + configurações.
- Mantém o layout mobile original.
