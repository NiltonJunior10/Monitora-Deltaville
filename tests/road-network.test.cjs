...
test('image-space network has unique versioned IDs and finite in-bounds vertices',()=>{
  ...
  for(const r of network.roads){
    assert.match(r.id,/-v1$/);assert.ok(r.points.length>=2);
    for(const [x,y] of r.points)assert.ok(Number.isFinite(x)&&Number.isFinite(y)&&x>=0&&x<=1601&&y>=0&&y<=982,r.id);
  }
});
test('Beira Rio snaps to each carriageway, never its median or Sunset plots',()=>{
  const run=geometry();
  assert.equal(run('nearestAvenueProjection({lat:382,lng:1132}).routeKey'),'beira-oeste-v1');
  assert.equal(run('nearestAvenueProjection({lat:382,lng:1172}).routeKey'),'beira-leste-v1');
  assert.equal(run('nearestAvenueProjection({lat:382,lng:1152})'),null);
...
test('offline shell includes the road module before app startup',()=>{
  ...
  assert.ok(html.indexOf('road-network.js?v=756')<html.indexOf('app.js?v=756'));
  assert.ok(sw.includes('"./road-network.js?v=756"'));
});
